...
> **Counter now 3** (row 10=1 → row 11=2 → row 12=3). Next mandatory §11.2 full-UI pass when the counter reads 4 (**1 handoff away** — the NEXT session is due). Row 12 was plist/Info-only and got a GUI launch eyeball, but a full §11.2 matrix pass is still owed next session.
