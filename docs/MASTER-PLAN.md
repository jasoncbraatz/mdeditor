...
| 19 | 2026-07-01 | **Phase 4 finding 9a — `Scripts/fuzz/run.sh` per-file timeout LANDED + Release redeployed to `/Applications/mdeditor.app`** (Strike-Zone worker, task 1216191462057469). (a) **9a per-file timeout:** wrapped every harness invocation in `perl -e 'alarm shift; exec @ARGV or die $!'` (macOS has no coreutils `timeout`); `FUZZ_TIMEOUT_S=15s` default (override via env, `0` disables). SIGALRM (14) → `rc=142` is caught by the existing `rc>=128` signal path, so hangs are handled exactly like crash-signals — either FAIL the gate as a new defect, or accept via `KNOWN_OPEN`. Effect: the gate is now BOUNDED (no more indefinite hangs on pathological input) and 7b-time-class DoS surfaces as a distinct signal with a `TIMEOUT ${FUZZ_TIMEOUT_S}s` label. (b) **Discovery:** the timeout wrapper immediately surfaced 3 additional latent 7b-time siblings the crash-only gate had silently hidden — `pmh_fuzz:deep_blockquote.md`, `pmh_fuzz:deep_list.md`, `hoedown_fuzz:deep_list.md` (`backtick_runs.md` was already known). All 4 `KNOWN_OPEN`'d as 7b-time-class pending a per-parser time-budget guard bundle. (c) **Redeployed `/Applications/mdeditor.app`** — the installed Release was severely stale (`0.8.0d61` build 1140, Jun 30 pre-Phase-4). Backed up to `~/.Trash/mdeditor.app.replaced-*` + `/tmp/mdeditor.app.pre-*.bak`; installed fresh Release build @ HEAD `4dc776e` (build 1156, `0.8.0d77`) carrying every Phase 4 fix (Sparkle removed, XSS 3-layer, ATS/plist hardening, Hardened Runtime, CVE-2014-2525 + 7a/7b-heap/7b-stack/7c caps); `lsregister -f` refresh. Left the 3 running Release instances alone (Jason's REPORT.md work). `Scripts/fuzz/run.sh` **PASS** (4 known-open 7b-time-class, 0 new); headless Debug **67/0**; Release **BUILD SUCCEEDED**. Tag `pre-7c-land-1216191462057469`. | headless **67/0** + fuzz `run.sh` PASS + Release redeploy verified (`defaults read` = 1156/0.8.0d77 in `/Applications/mdeditor.app`); §11.2 full-UI: NO (not due; counter=2) | 2 |
| 18 | 2026-07-01 | **Phase 4 finding 7c — LibYAML loader recursion FIXED & LANDED** (Strike-Zone worker, task 1216191462057445). Root: `Pods/LibYAML/src/loader.c`'s `yaml_parser_load_node` mutually recurses with `load_sequence`/`load_mapping` with **no document-depth limit**; on the 512 KB `NSOperationQueue` thread stack (`NSString+Lookup` → `YAMLSerialization` → `yaml_parser_load`), a 100 KB YAML front-matter of stacked `[`/`{` blows the stack. **Fix (mirrors CVE-2014-2525 pattern):** static compose-time depth counter capped at `MDEDITOR_YAML_MAX_DEPTH=100` around the `load_node` dispatch; on cap-hit `load_node` calls `yaml_parser_set_composer_error(parser,"YAML nesting exceeds mdeditor depth cap",…)` and returns 0 (the composer already treats 0 as failure, so it unwinds cleanly). Static counter is safe: MacDown drives one YAML parse at a time on a dedicated queue. Applied idempotently in `Podfile` `post_install` (anchor = the original `load_node` body) **and** directly in `Pods/LibYAML/src/loader.c`. Both PoCs (`deep_flow_seq.yaml`, `deep_flow_map.yaml`) now exit 0 under the ASan/UBSan `yaml_fuzz` harness; removed both from `run.sh` `KNOWN_OPEN` (both 7c *and* the residual 7b/7a entries clear); `Scripts/fuzz/run.sh` **PASS** (0 known-open, 0 new); the two positive controls (7a SIZE_MAX @512KB and 7b-stack uncapped @512KB) still overflow rc=138 (bus error) → the caps are load-bearing. Full Debug test suite **67/0**, `** TEST SUCCEEDED **`. **Finding 7's stack-overflow class is now fully closed** (7a + 7b-heap + 7b-stack + 7c all LANDED); only the softer, cancellable **7b-time** sibling (backtick_runs.md-class exponential backtracking) + finding 9's `run.sh` per-file timeout remain. Redeploy of Release to `/Applications/mdeditor.app` is bundled in this row. Tag `pre-7c`. | headless **67/0** + fuzz `run.sh` PASS; §11.2 full-UI: NO (not due; counter=1) | 1 |
