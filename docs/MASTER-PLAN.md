...
- **Parser fuzz tooling (finding 7, 2026-06-30):** `Scripts/fuzz/` — `build.sh` (ASan/UBSan
  ...
  lists accepted 7b-stack/7c — **7a + 7b-heap are FIXED** (7b-heap = val-stack grow-guard in
  `yyPush`, patched in both `pmh_parser.c` and the greg emitter `greg/compile.c`), the main hoedown loop runs at the product cap
