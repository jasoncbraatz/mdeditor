...
> **Counter now 2** (row 10=1 → row 11=2). Next mandatory §11.2 full-UI pass when the counter reads 4 (3 handoffs away). Row 11 touched the preview render path, so the live XSS eyeball doubles as a targeted UI spot-check.
